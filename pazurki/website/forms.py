...
class Add_user(UserCreationForm):
    ...
    is_staff = forms.ChoiceField(label="Is staff", choices=BOOL_YN,
                                 widget=forms.Select(attrs={'class': 'form-control', 'placeholder': 'Staff'}))
    ...
    class Meta:
        model = User
        fields = ['username', 'first_name', 'last_name', 'password1', 'password2', 'is_staff', 'is_active',
                  'is_superuser']
...
class AddAnimal(forms.ModelForm):
    ...
    status = forms.ChoiceField(required=True, label="", choices=STATUS_CHOICES,
                               widget=forms.Select(attrs={'class': 'form-control', 'placeholder': 'Status'}))
    breed = forms.CharField(required=True, widget=forms.widgets.TextInput(
        attrs={"placeholder": "Breed", "class": "form-control"}), label="")
    species = forms.ChoiceField(required=True, label="", choices=SPECIES_CHOICES,
                                widget=forms.Select(attrs={'class': 'form-control', 'placeholder': 'Species'}))
    sex = forms.ChoiceField(required=True, label="", choices=GENDER_CHOICES,
                            widget=forms.Select(attrs={'class': 'form-control', 'placeholder': 'Sex'}))
    age = forms.IntegerField(required=True,
                             widget=forms.widgets.NumberInput(attrs={"placeholder": "Age", "class": "form-control"}),
                             label="")
    size = forms.ChoiceField(required=True, label="", choices=SIZE_CHOICES,
                             widget=forms.Select(attrs={'class': 'form-control', 'placeholder': 'Size'}))
    vaccinations = forms.ChoiceField(label="Vaccinations:", choices=YN_CHOICES, widget=forms.Select(
        attrs={'class': 'form-control', 'placeholder': 'vaccinations'}))
    sterilization = forms.ChoiceField(label="Sterilization:", choices=YN_CHOICES, widget=forms.Select(
        attrs={'class': 'form-control', 'placeholder': 'sterilization'}))
    friendly_kids = forms.ChoiceField(label="Friendly to kieds:", choices=YN_CHOICES, widget=forms.Select(
        attrs={'class': 'form-control', 'placeholder': 'friendly_kids'}))
    friendly_others = forms.ChoiceField(label="Friendly to others:", choices=YN_CHOICES, widget=forms.Select(
        attrs={'class': 'form-control', 'placeholder': 'friendly_others'}))

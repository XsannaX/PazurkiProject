...
def add_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # login(request, user)
            messages.success(request, ("You have successfully registered new worker!"))
            return redirect('home')
...
def add_animal(request):
    form = AddAnimal(request.POST or None, request.FILES or None)
    if request.method == "POST":
        if form.is_valid():
...
def update_animal(request, pk):
    ...
    form = AddAnimal(request.POST or None, request.FILES or None, instance=current_record)
    if form.is_valid():
        form.save()
...
def view_adoption_form(request):
    ...
    adoption = Adopt_form.objects.filter(animal__status="For adoption")
...
def dogs(request):
    animals = Animal.objects.filter(species='dog', status="For adoption")
...
def cats(request):
    animals = Animal.objects.filter(species='cat', status="For adoption")
    myFilter = AnimalFilter(request.GET, queryset=animals)
    context = {'myFilter': myFilter}
    p = Paginator(myFilter.qs, 6)
    page_num = request.GET.get('page')
...
def under_my_care(request):
    all_animals = Connector.objects.filter(animal__status="For adoption")
